...
class Box
{
	...
	{
		$this->im = $image;
		$this->fontColor = imagecolorallocate($image, 0, 0, 0);
	}
	 ...
	 */
	public function setTextShadow($color, $xShift, $yShift)
	{
	...
	public function draw($text)
	{
		...
		if ($this->debug) {
			...
			$this->drawFilledRectangle(
				...
				$this->box['width'],
				$this->box['height'],
				imagecolorallocatealpha($this->im, rand(180, 255), rand(180, 255), rand(180, 255), 90)
			);
		}
		...
		foreach ($lines as $line) {
			...
			if ($this->debug) {
				...
				$this->drawFilledRectangle(
					...
					$boxWidth,
					$lineHeightPx,
					imagecolorallocate($this->im, rand(1, 180), rand(1, 180), rand(1, 180))
				);
			}
	...
	protected function drawFilledRectangle($x, $y, $width, $height, $color)
	{
	...
	protected function drawInternal($x, $y, $color, $text)
	{
		imagefttext(
			$this->im,
